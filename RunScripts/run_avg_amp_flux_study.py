...
if __name__ == '__main__':
	...
	for in_data, fig_name, data_labs in zip(input_data, figure_names, data_labels):
		...
		ax1 = fig.add_subplot(gs[0, 2])
		for p in range(1, 6 + 1):
			# ax1.plot(k[:-6], np.mean(np.power(np.absolute(np.cos(np.angle(in_data))), p/3.), axis=0)[:-6], label=r"$\arg $; $p = {}$".format(p))
		...
		# 	shells = np.arange(in_data.shape[-1] - 1)
		# 	# shells = [1 - 1, 5 - 1, 10 - 1, 15 - 1, 20 - 1]
		...
		# 			# Save figure
		# 			plt.suptitle(fig_name + " $n = {} $".format(n + 1))
		# 			fig.savefig(cmdargs.out_dir_AVGFLUX + fig_name + "_2DHist_Comparison_Un_n{}".format(n) + fig_format, bbox_inches='tight')
		# 			plt.close()
