...
plot_dir="/home/enda/PhD/Shell_Model/Data/DenseData/Plots/"
...
with h5py.File(true_out_dir_data[0]+"/Main_HDF_Data.h5",'r') as HDFfileRuntime:
	num_t = HDFfileRuntime["VelModes"].shape[0]
	## Get the Velocity Modes
	start  = 0
	stride = 1000
	count  = (num_t) // stride
	block  = 1	
	u = HDFfileRuntime["VelModes"][h5py.MultiBlockSlice(start, stride, count, block), :]
	num_t_steps, num_shell = u.shape
...
Eddy_Turn_all 	 = np.empty([true_data_no, num_t_steps], dtype=float, order='C')
...
for i in range(0, true_data_no):
	...
	with h5py.File(true_out_dir_data[i]+"/Main_HDF_Data.h5",'r') as HDFfileRuntime:
		...
		count  = (num_t) // stride
		block  = 1	
		tmp_u = HDFfileRuntime["VelModes"][h5py.MultiBlockSlice(start, stride, count, block), :]
		Tot_Enrg_all[i,  :]    = np.sum(np.absolute(tmp_u)**2, axis=-1)
		Eddy_Turn_all[i, :]    = 1.0 / (k[0] * np.absolute(tmp_u[:, 0]))
		Vel_Modse_all[i, :, :] = tmp_u[:, :]
# --------------------------------------------------
# # --------  Plotting Data
# --------------------------------------------------
print("Plotting Data")
...
mean_eddy_turn  = np.mean(Eddy_Turn_all[:, :], axis=0)
time = np.linspace(sys_vars.T * 0.2, sys_vars.T, num_t_steps)
...
ax1.plot(time, mean_eddy_turn, label=r"Mean = {}".format(np.mean(mean_eddy_turn)))
ax1.set_xlabel(r"$t$")
ax1.set_title(r"Ensemble Mean Eddy Turnover")
ax1.legend()
ax1.grid(which = "both", axis = "both", color = 'k', linestyle = ":", linewidth = 0.5)
fig.savefig(plot_dir + "System_Measures" + "." + fig_format, format=fig_format, bbox_inches='tight')
plt.close()
...
###------------------ Plot PDFs
plot_PDF_InOne(plot_dir + "EnsPDFs_Vel_Real" + "." + fig_format, np.real(Vel_Modse_all), lab="\Re u_n", remove_zeros=True)
