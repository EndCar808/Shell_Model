...
@njit
def nonlin_POAO(non_PO, non_AO, a, p, k, N, delta, lam, Mod_Type):
	for i in range(N):
		n = i + 2
		
		## Nonlinear Term
		if Mod_Type == "AMP_PHASE":
			non_AO[n] = k[n] * (a[n + 1] * a[n + 2] * np.sin(p[n + 1] * p[n + 2] + p[n]) - (delta/lam) * (a[n - 1] * a[n + 1] * np.sin(p[n - 1] + p[n + 1] + p[n])) - ((1.0 - delta)/lam**2) * (a[n - 2] * a[n - 1] * np.sin(p[n - 2] + p[n - 1] + p[n])))

			non_PO[n] = (k[n] / a[n]) * (a[n + 1] * a[n + 2] * np.cos(p[n + 1] * p[n + 2] + p[n]) - (delta/lam) * (a[n - 1] * a[n + 1] * np.cos(p[n - 1] + p[n + 1] + p[n])) - ((1.0 - delta)/lam**2) * (a[n - 2] * a[n - 1] * np.cos(p[n - 2] + p[n - 1] + p[n]))) 
		
		# ## Add Forcing
		# if n == forcing_shell + 2:
		# 	if Mod_Type == "AMP_PHASE":
		# 	  non_AO[n] += forcing_scale * (np.sqrt(2.0) * np.cos(np.pi/4.0 - p[n]))
		# 	  non_PO[n] += forcing_scale * (np.sqrt(2.0) * np.sin(np.pi/4.0 - p[n])) / a[n]
...
if model_type == "AMP_PHASE" or model_type == "AO" or model_type == "PO":
	if Printing:
		for i in range(N + 4):
			print("k[{}]: {}\tu[{}]: {:1.16f} {:1.16f}\ta[{}]: {:1.16f}\tp[{}]: {:1.16f}".format(i, k[i], i, np.real(amp[i] * np.exp(1j * phi[i])), np.imag(amp[i] * np.exp(1j * phi[i])), i, amp[i], i, phi[i]))
		print("\n")
else:
	if Printing:
		for i in range(N + 4):
...
if model_type == "AMP_PHASE" or model_type == "AO" or model_type == "PO":
	...
	if Printing:
		for i in range(N + 4):
			print("k[{}]: {}\tRK1[{}]: {:1.16f} {:1.16f}\ta[{}]: {:1.16f}\tp[{}]: {:1.16f}".format(i, k[i], i, np.real(a_rk1[i]* np.exp(1j * p_rk1[i])), np.imag(a_rk1[i]* np.exp(1j * p_rk1[i])), i, a_rk1[i], i, p_rk1[i]))
		print("\n")
else:
	rk1 = nonlin(rk1, u, k, N, delta, lam)
	if Printing:
		for i in range(N + 4):
			print("k[{}]: {}\tRK1[{}]: {:1.16f} {:1.16f}\ta[{}]: {:1.16f}\tp[{}]: {:1.16f}".format(i, k[i], i, np.real(rk1[i]), np.imag(rk1[i]), i, np.absolute(rk1[i]), i, np.angle(rk1[i])))
		print("\n")
...
while t <= 1 * dt:
	...
	if Printing:
		for i in range(N + 4):
			print("in[{}]: {} {}".format(i, np.real(u_tmp[i]), np.imag(u_tmp[i])))
		print("\n\n")
	...
	if solver_type == "INT_FAC":
		...
		if model_type == "AMP_PHASE":
			...
			if Printing:
				for i in range(N + 4):
					print("k[{}]: {}\tRK1[{}]: {:1.16f} {:1.16f}\ta[{}]: {:1.16f}\tp[{}]: {:1.16f}".format(i, k[i], i, np.real(a_rk2[i]* np.exp(1j * p_rk2[i])), np.imag(a_rk2[i]* np.exp(1j * p_rk2[i])), i, a_rk2[i], i, p_rk2[i]))
				print("\n")
		else:
		...
		if model_type == "AMP_PHASE":
			...
			if Printing:
				for i in range(N + 4):
					print("k[{}]: {}\tRK1[{}]: {:1.16f} {:1.16f}\ta[{}]: {:1.16f}\tp[{}]: {:1.16f}".format(i, k[i], i, np.real(a_rk3[i]* np.exp(1j * p_rk3[i])), np.imag(a_rk3[i]* np.exp(1j * p_rk3[i])), i, a_rk3[i], i, p_rk3[i]))
				print("\n")
		else:
		...
		if model_type == "AMP_PHASE":
			...
			if Printing:
				for i in range(N + 4):
					print("k[{}]: {}\tRK1[{}]: {:1.16f} {:1.16f}\ta[{}]: {:1.16f}\tp[{}]: {:1.16f}".format(i, k[i], i, np.real(a_rk4[i]* np.exp(1j * p_rk4[i])), np.imag(a_rk4[i]* np.exp(1j * p_rk4[i])), i, a_rk4[i], i, p_rk4[i]))
				print("\n")
		else:
		...
		if model_type == "AMP_PHASE":
			...
			if Printing:
				...
				for i in range(N + 4):
					if i >= 2 and i < N + 2:
						print("unew[{}]: {:1.16f} {:1.16f}".format(i - 2, np.real(amp[i] * np.exp(1j * phi[i])), np.imag(amp[i] * np.exp(1j * phi[i]))))
				print("\n")
		else:
			...
			if Printing:
				for i in range(N + 4):
					if i >= 2 and i < N + 2:
						print("unew[{}]: {:1.16f} {:1.16f}".format(i - 2, np.real(u[i]), np.imag(u[i])))
				print("\n")
	...
	##-----------------------------------------
		...
		if model_type == "AMP_PHASE":
			...
			if Printing:
				for i in range(N + 4):
					print("k[{}]: {}\tnu: {:g}\tRK1[{}]: {:1.16f} {:1.16f}\tRKTmp[{}]: {:1.16f} {:1.16f}".format(i, k[i], nu, i, np.real(a_rk1[i]* np.exp(1j * p_rk1[i])), np.imag(a_rk1[i]* np.exp(1j * p_rk1[i])), i, np.real(a_tmp[i] * np.exp(1j *p_tmp[i])), np.imag(a_tmp[i] * np.exp(1j * p_tmp[i]))))
				print("\n")
		else:
			rk1 = nonlin(rk1, u_tmp, k, N, delta, lam)
			rk1 -= nu * k * k * u 
		...
		if model_type == "AMP_PHASE":
			...
			if Printing:
				for i in range(N + 4):
					print("k[{}]: {}\tRK2[{}]: {:1.16f} {:1.16f}\tRKTmp[{}]: {:1.16f} {:1.16f}".format(i, k[i], i, np.real(a_rk2[i]* np.exp(1j * p_rk2[i])), np.imag(a_rk2[i]* np.exp(1j * p_rk2[i])), i, np.real(a_tmp[i] * np.exp(1j *p_tmp[i])), np.imag(a_tmp[i] * np.exp(1j * p_tmp[i]))))
				print("\n")
		else:
			rk2    = nonlin(rk2, u_tmp, k, N, delta, lam)
			rk2 -= nu * k * k * u 
		...
		if model_type == "AMP_PHASE":
			...
			if Printing:
				for i in range(N + 4):
					print("k[{}]: {}\tRK3[{}]: {:1.16f} {:1.16f}\tRKTmp[{}]: {:1.16f} {:1.16f}".format(i, k[i], i, np.real(a_rk3[i]* np.exp(1j * p_rk3[i])), np.imag(a_rk3[i]* np.exp(1j * p_rk3[i])), i, np.real(a_tmp[i] * np.exp(1j *p_tmp[i])), np.imag(a_tmp[i] * np.exp(1j * p_tmp[i]))))
				print("\n")
		else:
			rk3    = nonlin(rk3, u_tmp, k, N, delta, lam)
			rk3 -= nu * k * k * u 
		...
		if model_type == "AMP_PHASE":
			...
			if Printing:
				for i in range(N + 4):
					print("k[{}]: {}\tRK4[{}]: {:1.16f} {:1.16f}".format(i, k[i], i, np.real(a_rk4[i]* np.exp(1j * p_rk4[i])), np.imag(a_rk4[i]* np.exp(1j * p_rk4[i]))))
				print("\n")
		else:
			rk4    = nonlin(rk4, u_tmp, k, N, delta, lam)
			rk4 -= nu * k * k * u
		...
		if model_type == "AMP_PHASE":
			...
			if Printing:
				...
				for i in range(N + 4):
					if i >= 2 and i < N + 2:
						print("RK4unew[{}]: {:1.16f} {:1.16f}".format(i - 2, np.real(amp[i] * np.exp(1j * phi[i])), np.imag(amp[i] * np.exp(1j * phi[i]))))
				print("\n")
		else:
