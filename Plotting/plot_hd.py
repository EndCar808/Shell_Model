...
if __name__ == '__main__':
	...
	if cmdargs.plotting is True:
		...
		ax1 = fig.add_subplot(gs[0, 0])
		for i in range(3):
			ax1.plot(sys_msr_data.time, np.real(run_data.u[:, i]), label=r"$n = {}$".format(i + 1))
		ax1.set_xlabel(r"$t$")
		ax1.set_title(r"Real U_n")
		ax1.grid(which = "both", axis = "both", color = 'k', linestyle = ":", linewidth = 0.5)
		ax1.legend()
		ax2 = fig.add_subplot(gs[1, 0])
		for i in range(3):
			ax2.plot(sys_msr_data.time, np.imag(run_data.u[:, i]), label=r"$n = {}$".format(i + 1))
		ax2.legend()
		ax2.set_xlabel(r"$t$")
		ax2.set_title(r"Imag U_n")
		ax2.grid(which = "both", axis = "both", color = 'k', linestyle = ":", linewidth = 0.5)
		ax4 = fig.add_subplot(gs[0, 1])
		ax4.plot(np.real(sys_msr_data.forcing_u[:]))
		ax4.set_xlabel(r"$t$")
		ax4.set_title(r"Real f_n")
		ax3 = fig.add_subplot(gs[1, 1])
		ax3.plot(np.imag(sys_msr_data.forcing_u[:]))
		ax3.set_xlabel(r"$t$")
		ax3.set_title(r"Imag f_n")
		# ax4 = fig.add_subplot(gs[0, 2])
		# ax4.plot(sys_msr_data.time, np.sum(np.real(run_data.u[:, :] * np.conjugate(sys_msr_data.forcing_u[np.newaxis, :]))))
		# ax4.set_xlabel(r"$t$")
		# ax4.set_title(r"Real Input")
		# ax4 = fig.add_subplot(gs[1, 2])
		# ax4.plot(sys_msr_data.time, np.sum(np.imag(run_data.u[:, :] * np.conjugate(sys_msr_data.forcing_u[np.newaxis, :]))))
		# ax4.set_xlabel(r"$t$")
		# ax4.set_title(r"Imag Input")
		ax4.grid(which = "both", axis = "both", color = 'k', linestyle = ":", linewidth = 0.5)
		fig.savefig(cmdargs.out_dir_HD  + "Forcing_Measures" + "." + fig_format, format=fig_format, bbox_inches='tight')
		plt.close()
		...
		# # --------  Plot Field PDFs
		# ---------------------------------------------------
		###------------------------ Individual Tseries --- Phases
		print("Plotting --- Phases T-series")
		num_bins = 1000
		norm_hist = False
		if sys_vars.model_type == "AO" or sys_vars.model_type == "PO" or sys_vars.model_type == "AOFXDPHASE":
			phi_data = np.mod(run_data.phi_n, 2.0*np.pi) - np.pi
		else:
			phi_data = np.mod(np.angle(run_data.u) + 2.0 * np.pi, 2.0 * np.pi) - np.pi
		fig_name = r"U_Phases"
		data_labs = r"$\arg\{ u_n \}$"
		...
		for i in range(5):
		  for j in range(5):
		      ...
		      if indx < phi_data.shape[-1]:
		          ...
		          p, = ax1.plot(sys_msr_data.time, phi_data[:, indx], label="$n = {}$".format(indx + 1))    
		          ax1.set_ylabel(data_labs)
		          ax1.set_xlabel(r"$t$")
		          ax1.set_ylim(-np.pi, np.pi)
		          # ax1.set_xticks([0.0, np.pi/2.0, np.pi, 1.5*np.pi, 2.0 * np.pi])
		          # ax1.set_xticklabels([r"$0$", r"$\frac{\pi}{2}$", r"$\pi$", r"$\frac{3\pi}{2}$", r"$2 \pi$"])
		          ax1.set_title("n = {}".format(indx + 1))
		fig.savefig(cmdargs.out_dir_HD  + fig_name + "_Tseries" + "." + fig_format, format=fig_format, bbox_inches='tight')
		plt.close()

		###------------------------ Individual Tseries --- Amps
		if sys_vars.model_type == "AO" or sys_vars.model_type == "PO" or sys_vars.model_type == "AOFXDPHASE":
		...
		for i in range(5):
		  for j in range(5):
		      ...
		      if indx < phi_data.shape[-1]:
		          ax1 = fig.add_subplot(gs[i, j])
		          p, = ax1.plot(sys_msr_data.time, amp_data[:, indx], label="$n = {}$".format(indx + 1))    
		          ax1.set_ylabel(data_labs)
		          ax1.set_xlabel(r"$t$")
		          # ax1.set_xticks([0.0, np.pi/2.0, np.pi, 1.5*np.pi, 2.0 * np.pi])
		          # ax1.set_xticklabels([r"$0$", r"$\frac{\pi}{2}$", r"$\pi$", r"$\frac{3\pi}{2}$", r"$2 \pi$"])
		...
		for i in range(5):
			for j in range(5):
				...
				if indx < phi_data.shape[-1]:
					ax1 = fig.add_subplot(gs[i, j])
					pdf, centres = compute_pdf(np.mod(phi_data[:, indx], 2.0 * np.pi), nbins=num_bins, bin_lims = (0.0, 2.0 * np.pi), normed=norm_hist)
		...
		for i in range(5):
		  for j in range(5):
		      ...
		      if indx < amp_data.shape[-1]:
		          ...
		          pdf, centres = compute_pdf(amp_data[:, indx], nbins=num_bins, normed=norm_hist)
		...
		###################################
		if os.path.exists(cmdargs.out_dir + "Phase_Sync_HDF_Data.h5"):
			if sys_vars.model_type == "PO" or sys_vars.model_type == "AO" or sys_vars.model_type == "AOFXDPHASE":
				phases = run_data.phi_n
			else:
				phases = np.mod(np.angle(run_data.u) + 2.0*np.pi, 2.0*np.pi)

			if sys_vars.model_type == "PO" or sys_vars.model_type == "AO" or sys_vars.model_type == "FULL" or sys_vars.model_type == "AOFXDPHASE":
